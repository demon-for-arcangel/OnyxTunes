...
const deleteAlbum = async (req, res) => {
  ...
  try {
    if (!Array.isArray(albumsIds) || albumsIds.length === 0) {
      return res
        .status(400)
        .json({ msg: "No se proporcionaron IDs de álbum para eliminar." });
    }
  ...
  try {
    ...
    if (albums.length === 0) {
      ...
      return res
        .status(200)
        .json({
          msg: "No se encontraron álbumes para el usuario.",
          albums: [],
        });
    }
    res.status(200).json({ msg: "Álbumes encontrados", albums });
  } catch (error) {
